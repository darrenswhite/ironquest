...
public class QuestDeserializer implements JsonDeserializer<Quest> {
   ...
   * The logger
   */
  private static final Logger LOG = LogManager.getLogger(QuestDeserializer.class);
  ...
  public Quest deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) {
    ...
    JsonObject requirementsObject = quest.getAsJsonObject(KEY_REQUIREMENTS);
    // Get the requirements object
    JsonObject ironmanRequirementsObject = quest.getAsJsonObject(KEY_REQUIREMENTS_IRONMAN);
    // Get the requirements object
    JsonObject recommendedRequirementsObject = quest.getAsJsonObject(KEY_REQUIREMENTS_RECOMMENDED);
    ...
    int questPoints = 0;
    Map<Skill, Integer> skillRewards = new EnumMap<>(Skill.class);
    ...
    if (ironmanRequirementsObject != null) {
      Set<Requirement> ironman = parseRequirements(ironmanRequirementsObject);
    ...
    if (recommendedRequirementsObject != null) {
      Set<Requirement> recommended = parseRequirements(recommendedRequirementsObject);
  ...
  private Set<Requirement> parseRequirements(JsonObject requirementsObject) {
    ...
    for (Map.Entry<String, JsonElement> e : requirementsObject.entrySet()) {
      ...
      if (skill.isPresent()) {
        ...
        requirements.add(new SkillRequirement(skill.get(), value.getAsInt()));
        ...
        // Add the quest ids
        questIds.forEach(q -> requirements.add(new QuestRequirement(q.getAsInt())));
      } else {
        // Other requirements
        switch (key) {
